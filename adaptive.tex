...
\end{center}
\caption{Results from an experiment comparing the performance of $k$-NN
  on the MNIST data set for different levels of random label noise $p$
  ...
  severely of a too-small value of $k$ is used. The table on the right {\bf (b)}
  shows that AKNN, with a fixed value of $A$, performs almost as well
...
We present a few preliminary  experiments using real-world
data. 
For our experiments we made a few practical alterations to the algorithm
described in figure~\ref{fig:alg}.

%\begin{itemize}
%\item
{\bf First,} we extend the algorithm to
multi-class. Suppose the set of possible labels is  $\labels$. We
replace the binary rule ``find the smallest $k$ such that $\left|\eta_n(B_k(x))\right| >
\Delta(n, k,\delta)$ with the rule: ``find the smallest $k$ such that
for some $y \in \labels$, $\left|\eta^y_n(B_k(x))\right| - \frac{1}{|\labels|}>
\Delta(n, k,\delta)$ where $\eta^y_n(S) \doteq \frac{\#_n\{x_i \in S
    \mbox{ and } y_i = y\}}{\#_n(S)}$ \newline
{\bf Second,} we replace Equation~(\ref{eq:delta-default}) with a simple one:
$\Delta = \frac{A}{\sqrt{k}}$, where $A$ is a {\em confidence}
parameter. \newline
{\bf Third,} our algorithm can output answers that are not a single
label. The output can be ``?'', which indicates that no label has
sufficient evidence. It can also be a subset of $\labels$ that
contains more than one element, indicating that that more than one
label has significant evidence. In some situations using subsets of
the labels is more infrmative. However, when we want to compare
head-to-head with $k$-NN we need to output a single label. We use a
simple heuristic to break such ties. The heuristic is to use the label
$y \in \labels$ for which $\max_k \eta^y_n(B_k(x))/\sqrt{k}$ is
largest.
%  \end{itemize}
...
\begin{itemize}
\item AKNN performs almost as well as $k$-NN with the optimal choice
  of $k$
\item On some datasets AKNN is {\em better} than $k$-NN with the
  optimal choice of $k$. We explain this by the ability of $AKNN$ to
  use different values of $k$ for different parts of the data.
\item AKNN has a natural ``confidence'' parameter, which can be used
...
\begin{figure}
    ...
    \end{subfigure}
  \centering
\caption{Figures depicting the performance of AKNN on notMNIST.
  ...
  dotted horizontal line represents the best accuracy that is achieved
  by $k$-NN for $k \approx 10$. The three colored curves on the LHS
  represents the accuracy of AKNN for three different values of the
  confidence parameter $A$. The accuracy at a given $k$ is the
  accuracy for the set of examples where AKNN chose the number of
  neighbors to be smaller or equal to $k$. The graphs on the RHS
  represent the converage of the same AKNN runs. When $A$ is 3 or 9,
  smaller values of $k$ yield no coverage. Observe that the curvess on
  the LHS start at the values of $k$ that the coverage increases from
  zero. Of particular interest is the red curve, that corresponds to
  AKNN wirth $A=1$. This curves reaches full coverage when $k$ is
  unrestricted and, when unrestricted, achieves higher accuracy the
  best accuracy achieve by $ki$-NN.}
