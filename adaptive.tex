...
which can also be phrased more similarly to \Cref{thm:general-error-bound}) as follows:
\begin{theorem}\label{thm:fastrate2}
Let $\delta$ be the confidence parameter of the adaptive nearest neighbor algorithm,
and assume that the family of all balls in $(X,d)$ has VC dimension $d_0$.
Then, with probability at least $1-\delta$ over the choice of training points,
\[ \pr_{X}(g_n(X) \neq g^*(X)) \ \leq \ \mu(\partial_n) .\]
...
\end{theorem}
The difference with \Cref{thm:general-error-bound} is that $\delta$ term is removed.
...
\[\mu(\partial_n) \leq \mathrm{poly}\left(\log(1/\delta), n\right),
\]
...
Given VC dimension bound $d_0$ and parameter $\delta$, define the high-probability-convergence versions of $\X^{\pm}_n$ as:
\begin{itemize}
\item 
$\X^+_n (\delta)$ of points $x \in \supp(\mu)$ with $\eta(x) > 1/2$, for which $\exists p_x > 0$ such that
\begin{align*}
\eta(B(x,r_{p_x}(x))) \geq \frac{1}{2} + c_2 \sqrt{\frac{d_0 \log n + \log (1/\delta)}{n p_x}}
\qquad\text{and}\qquad
\eta(B(x,r)) \geq 1/2 \quad\forall\quad 0 < r \leq r_{p_x}(x)
\end{align*}
where $c_2 = \max(2c_1, \frac{\varphi}{2} ) \sqrt{1 + c_0}$ for $\varphi := \frac{1}{2} (1 + \sqrt{5})$.
\item
$\X^-_n (\delta)$ defined symmetrically
\item
$\partial_n (\delta) = \X \setminus (\X^+_n (\delta) \cup \X^-_n (\delta))$
\end{itemize}
Define $\gamma_{n, \delta} := d_0 \log n + \log (1/\delta)$. %\akshay{We would like this to be $d_0 \log n + \log (1/\delta)$; the difference is in \Cref{lemma:points-in-balls}.} 
Finally, a version of \Cref{lemma:points-in-balls} from \cite{ChaudhuriDasgupta2010} is useful:
...
$$ \mu(B) \geq \frac{k}{n} + \frac{c_0}{n} \left( \gamma_{n, \delta} + \sqrt{k \gamma_{n, \delta}} \right)
\ \ \implies \ \ 
\mu_n(B) \geq \frac{k}{n} $$
\end{lemma}

\begin{proof}[Proof of \Cref{thm:fastrate2}]
We adapt the argument of \Cref{lemma:good-sets}. 
...
\begin{enumerate}
\item For any integer $k$, we have $\mu_n(B) \geq k/n$ whenever $n \mu(B) \geq k + c_0 \left( \gamma_{n, \delta} + \sqrt{k \gamma_{n, \delta}} \right)$
\item $|\bias_n(B) - \bias(B)| \leq \Delta(n, n \mu_n(B)) = c_1 \sqrt{\frac{\gamma_{n, \delta}}{n \mu_n(B)}}$
\end{enumerate}
...
The argument is analogous to that of \Cref{lemma:good-sets}, with $\log (n/\delta)$ replaced by $\gamma_{n, \delta}$. We state the argument for $x \in \X^+_n (\delta)$. Define $p(x) = p_x$ to be a function of $x$ as specified in the definition of $\X^+_n (\delta)$, and set $k(x) = np(x) / (1 + c_0)$. We first observe that by definition of $p(x)$ and $c_2$, we have $c_2 \sqrt{\gamma_{n, \delta}/(n p(x))} \leq 1/2$, and thus $np(x) \geq 4 c_2^2 \gamma_{n, \delta} \geq \varphi^2 (1+c_0) \gamma_{n, \delta}$, so that $k(x) \geq \varphi^2 \gamma_{n, \delta}$. Therefore, $\frac{1}{c_0} (np(x) - k(x)) = k(x) = \left( \frac{1}{\varphi^2} + \frac{1}{\varphi} \right) k(x) \geq \gamma_{n, \delta} + \sqrt{ \gamma_{n, \delta} k(x) }$. Thus, by property 1, the ball $B = B(x, r_{p(x)} (x))$ has $\mu_n(B) \geq k(x)/n$. This means, in turn, that by property 2, 
\begin{align*}
\bias_n(B) &\geq \bias(B) - \Delta(n, k(x) ) 
\geq c_2 \sqrt{\frac{\gamma_{n, \delta}}{np}} - c_1 \sqrt{\frac{\gamma_{n, \delta}}{k(x)}} 
\geq 2c_1 \sqrt{\frac{\gamma_{n, \delta}}{k(x)}} - c_1 \sqrt{\frac{\gamma_{n, \delta}}{k(x)}} 
\geq \Delta(n, n \mu_n(B))
\end{align*}
So ball $B$ would trigger a prediction of $1$.
At the same time, for any ball $B' = B(x, r)$ with $r < r_{p(x)} (x)$,
$$ \bias_n(B') \geq \bias(B') - \Delta(n, n \mu_n(B')) > -\Delta(n, n \mu_n(B')) $$
and thus no such ball will trigger a prediction of $0$. Therefore, the algorithm's prediction at $x$ must be $1$.
