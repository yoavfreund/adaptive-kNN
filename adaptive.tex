...
if no such $k$ exists, then ``?'' (``don't know'') is output.  
...
\end{itemize}
\vspace{.1in}
\item To predict at $x \in \X$:
\begin{itemize}
\item For any integer $k$, let $B_k(x)$ denote the smallest ball centered at $x$ that contains $k$ training points.~\footnote{In cases where a set ($A$) of two or more points are at the same distance from $x$ our algorithm skips values of $k$ that corresponds to strict subsets of $A$.}
...
\begin{equation}
\eta_n^{y} (B_k(x)) - \frac{1}{|\mathcal{Y}|} > \frac{A}{\sqrt{k}}.
...
The discussions of Section \ref{sec:knn-comparison} show formally that $\algname$ compares favorably to $k$-NN with any fixed $k$. We see this in practice as well (Fig. \ref{fig:aknnvsknn}). 
...
&= c_1 \sqrt{\frac{\log (n/\delta)}{k}} = \Delta(n, \#_n(B), \delta) .
\end{align*}
